...
## Ray Minimum Distance / `minpos()` / `xyzdist()` in VEX

Just like `primuv()`, I had to remake `xyzdist()` and `minpos()` in OpenCL too. Sadly  `xyzdist()` uses an acceleration structure (likely BVH), which I can't recreate easily in VEX or OpenCL.

Therefore, the functions below only work when you already know the primnum. They return the nearest position and UVW coordinate to that primitive.
