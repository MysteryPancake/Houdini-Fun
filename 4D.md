...
- It subtracts the radius so the inside of the sphere is negative (hence 'signed distance').
...
Generate a bunch of random points in 4D, then snap them to the surface.

### 1. Generate random 4D points
This can be done with Point Replicate or Points from Volume, but I did it manually for fun.

Here's generating points in a sphere shape. It's faster to use a Numbers wrangle so it runs in parallel.

```js
// Pick a random 4D position in a sphere at {0, 0, 0, 0} with radius 1
float sphereRadius = 1.0;
vector4 randPos = sample_hypersphere_uniform(rand(i@elemnum)) * sphereRadius;
...
float radius = fit01(rand(i@elemnum + 1), 0.01, 0.05);
setpointattrib(0, "pscale", id, radius);
```

<img src="./images/4d/4dpointsphere.png" height="400">

Another option is generating points in a box shape.
