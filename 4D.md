...
This means we get perfect normals with great performance, since we only evaluate the SDF once.
...
The change in distance tells us the direction towards the surface, which tells us the normal of that surface.
...
Afterwards you can use the central difference to find the gradient, just like the Trail node in Houdini.
...
If we repeat this for all coordinates, we get a 4D version of [Inigo's normal function](https://iquilezles.org/articles/normalsSDF/).
