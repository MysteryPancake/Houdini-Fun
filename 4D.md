...
Note this assumes the SDF is perfectly Euclidean, which is usually not the case (for example after using smooth min).
...
SDF boolean is much faster than polygonal boolean, so don't be afraid to use it!
