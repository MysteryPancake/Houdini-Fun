...
In the code from before, the W slider sets the position of the slice along the W axis.
...
To get something more interesting, we should transform the cube in 4D!

## 4D transformations
Keep in mind I wrote this section for 4D SDFs, so this might not work in other situations.
...
### Scale
Scale is a bit weird when working with SDFs, since you need to evaluate a smaller version of the SDF.
