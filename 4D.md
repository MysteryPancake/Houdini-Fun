...
This heavily depends on each axis, so it needs another change to include the W axis:
...
Since we're using SDFs we have two options: smart and brute force.
...
<br>
This means you get perfect normals with great performance, since you only need to sample the SDF once.

However, you need to generalize all the gradient functions to 4D. I'm lazy so I went with the second option.
...
There's many ways to snap to 3D surfaces in Houdini, like Ray set to 'Minimum Distance' or `v@P = minpos(1, v@P)`.
<br>
Luckily it's just as easy to snap to 4D surfaces. We just need the normal and the distance.

The normal tells us the direction to travel, and the SDF how far. Combine these and we hit the surface.

```js
vector4 p = set(v@P.x, v@P.y, v@P.z, w);

// Find the direction and distance to the nearest 4D surface
float dist = sdf(p);
